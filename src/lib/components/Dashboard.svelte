<script>
    ...
    $: stats = getStats($transactions, $accounts);
    // $: pl = stats.investmentValue - stats.investmentCost; // Removed
    $: savingsRate =
        stats.totalIncome > 0
            ? Math.round(
    ...
    function initCharts() {
        ...
        if (canvasAccounts) {
            ...
            // const invVal = stats.investmentValue; // Removed
            if (saldoLiquido > 0) {
                chartAccounts = new Chart(canvasAccounts, {
                    ...
                    data: {
                        ...
                        datasets: [
                            {
                                data: [saldoLiquido > 0 ? saldoLiquido : 0],
        ...
        if (canvasBalances) {
            const activeAccounts = Object.entries(stats.accounts).filter(
                ([name, bal]) => bal !== 0,
            );
                        ...
                        datasets: [
                            {
                                ...
                                backgroundColor: activeAccounts.map((a) =>
                                    a[1] >= 0 ? "#3b82f6" : "#ef4444",
                                ),
                                borderRadius: 4,
                            },
...
<div class="space-y-6 fade-in pb-10">
    <!-- AI Advisor Card -->
    <div
        class="bg-gradient-to-r from-slate-800 to-indigo-900 rounded-2xl p-6 text-white shadow-xl shadow-indigo-900/20 relative overflow-hidden"
    >
        <div class="absolute top-0 right-0 p-8 opacity-10">
            <BrainCircuit class="w-32 h-32" />
        </div>
        <div
        ...
        >
            <div>
                ...
                <p class="text-indigo-200 mt-1 text-sm max-w-md">
                    Análisis profundo de tus patrones de gasto y salud
                    financiera.
                </p>
                <div class="mt-2"><QuotaBadge /></div>
            </div>
            <button
                on:click={generateAIReport}
                disabled={$aiLoading}
                class="bg-white/10 backdrop-blur-md border border-white/20 text-white hover:bg-white/20 px-5 py-2.5 rounded-xl font-medium shadow-lg text-sm transition-all flex items-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed"
            >
                {#if $aiLoading}
                    <div
                        class="loader border-white border-t-transparent mr-2"
                    ></div>
                     Analizando...
                {:else}
                    <Sparkles
        ...
        {#if $aiReportText}
            <div
                class="mt-6 bg-black/20 backdrop-blur-sm rounded-xl p-5 border border-white/10 text-sm leading-relaxed font-light tracking-wide text-indigo-50 relative fade-in markdown-body ai-report-dark"
            >
                <button
                    on:click={clearAIReport}
                    class="absolute top-3 right-3 text-indigo-300 hover:text-white p-1 rounded-full hover:bg-white/10 transition-colors"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        <!-- Card 1 -->
        <div
            class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 relative overflow-hidden group hover:shadow-md transition-all"
        >
            <div
                class="absolute right-0 top-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity text-slate-600"
            >
                <Wallet class="w-16 h-16" />
            </div>
            <div
                class="text-slate-500 text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-2"
            >
                <Wallet class="w-4 h-4 text-slate-500" /> Patrimonio
            </div>
            <div class="text-2xl font-bold text-slate-800 tracking-tight">
                ${stats.totalNetWorth.toLocaleString()}
            </div>
        ...
        >
            <div
                class="absolute right-0 top-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity text-emerald-600"
            >
            ...
            <div
                class="text-slate-500 text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-2"
            >
                <Activity class="w-4 h-4 text-emerald-500" /> Flujo
            </div>
            <div class="text-2xl font-bold text-slate-800 tracking-tight">
                ${stats.balance.toLocaleString()}
            </div>
            <div class="text-xs text-slate-400 mt-1 font-medium">
                Ingresos: ${stats.totalIncome.toLocaleString()}
            </div>
        </div>
        <!-- Card 3 -->
        <!-- Card 3 Removed -->
        ...
        >
            <div
                class="absolute right-0 top-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity text-blue-600"
            >
                <PiggyBank class="w-16 h-16" />
            </div>
            <div
                class="text-slate-500 text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-2"
            >
                <PiggyBank class="w-4 h-4 text-blue-500" /> Ahorro
            </div>
            <div class="text-2xl font-bold text-slate-800 tracking-tight">
                {savingsRate}%
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        <div
            class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 h-80"
        >
            <h3
                class="font-bold text-slate-800 mb-6 text-sm flex items-center gap-2"
            >
                <div class="w-1 h-4 bg-emerald-500 rounded-full"></div>
                 Flujo de Caja
            </h3>
            <div class="relative h-60 w-full">
                <canvas bind:this={canvasCashFlow}></canvas>
            </div>
        </div>
        <div
            class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 h-80"
        >
            ...
            <div class="relative h-60 w-full">
                <canvas bind:this={canvasBalances}></canvas>
            </div>
        </div>
        ...
        >
            <h3
                class="font-bold text-slate-800 mb-6 text-sm flex items-center gap-2"
            >
                <div class="w-1 h-4 bg-indigo-700 rounded-full"></div>
                 Activos vs Pasivos (Aprox)
            </h3>
            <div class="relative h-60 w-full flex justify-center">
                <canvas bind:this={canvasAccounts}></canvas>
            </div>
        </div>
        ...
        >
            ...
            <div class="relative h-60 w-full flex justify-center">
                <canvas bind:this={canvasExpenses}></canvas>
            </div>
        </div>
    </div>
</div>
